...
title: about
layout: about
date: 2026-03-15 18:00:01
---

# 我是
会压网线和3D建模的云原生运维开发，理论上应该常住深圳，但目前在上海当便宜牛马 (截止2026本博文更新当天)。


# 关于网站
博客部署在阿里云服务器，采用Hexo&Fluid搭建，代码分离为了主仓库（代码仓库）、子仓库（文档仓库）放置在了Github。
后台系统采用私有部署的[onedev](https://onedev.io/)实现代码托管与CI/CD。~~（原先是使用GitLab进行托管和CI/CD的，但是由于沟槽的国区迁移令而放弃了。）~~

后台系统采用Vue3&TypeScript-Vben框架完成前端部分开发，Python3-DjangoRestFramework框架完成后端部分开发，MySQL数据库存储，Nginx做代理转发。
同时基于Golang的net/http标准库实现了服务端的SSE应用，完成了工作流结束时的双向消息通知。
...
![整体架构](https://fishingrodd.cn/img/architecture.png)
...
## 后台系统
![后台PC工作台-阿里云OpenAPI、百度统计API、和风天气API接入](https://fishingrodd.cn/img/demo1.png)
![后台PC工作台 - 最近访问动态](https://fishingrodd.cn/img/demo2.png)
![后台PC工作台 - CICD管理](https://fishingrodd.cn/img/cicd.png)
![移动端PC工作台](https://fishingrodd.cn/img/demo3.jpg)
